...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 최초 경로 탐색
        actions = bfs(mapData, start, target, WALL_SYMBOL, WATER_SYMBOL, DIRS, MOVE_CMDS, FIRE_CMDS);
        ...
        while (gameData != null && gameData.length() > 0) {
            ...
            if (actions.isEmpty()) {
                ...
                actions = (start != null && target != null)
                        ? bfs(mapData, start, target, WALL_SYMBOL, WATER_SYMBOL, DIRS, MOVE_CMDS, FIRE_CMDS)
                        : new LinkedList<>();
            }
        ...
        while (!queue.isEmpty()) {
            ...
            for(int d = 0; d< dirs.length;d++){
                ...
                for(int dist = 1; dist<=3; dist++){
                    ...
                    if(grid[nr][nc].equals(wall) || grid[nr][nc].equals(water)) break;
            ...
            // 1. 현재 위치에서 사격 가능 여부 확인 (사거리 3)
//            for (int d = 0; d < dirs.length; d++) {
//                int nr = r;
//                int nc = c;
//                for (int dist = 1; dist <= 3; dist++) {   // 최대 사거리 3
//                    nr += dirs[d][0];
//                    nc += dirs[d][1];
//
...
//                        Queue<String> result = new LinkedList<>(actions);
//                        result.offer(fireCmds[d]);
//                        return result;  // BFS니까 최단 경로 + 사거리 조건 만족
//                    }
//                }
//            }
            ...
            for (int d = 0; d < dirs.length; d++) {
                ...
                if (nr >= 0 && nr < rows && nc >= 0 && nc < cols &&
                        grid[nr][nc] != null &&
                        !grid[nr][nc].equals(wall) &&
                        !grid[nr][nc].equals(water) &&
                        !visited.contains(key)) {
    ...
    private static void printData(String gameData) {
        ...
        for (String key : myAllies.keySet()) {
            String[] value = myAllies.get(key);
            if (key.equals("M")) {
                System.out.printf("M (내 탱크) - 체력: %s, 방향: %s, 일반포탄: %s, 메가포탄: %s\n",
                        value[0], value[1], value[2], value[3]);
            } else if (key.equals("H")) {
                System.out.printf("H (아군 포탑) - 체력: %s\n", value[0]);
            } else {
